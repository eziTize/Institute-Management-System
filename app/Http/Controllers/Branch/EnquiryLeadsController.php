...
class EnquiryLeadsController extends Controller
{
    ...
    */
    public function index(){
        $data = [
            'data' => EnquiryLeads::getAllEnquiry('branch'),
            ...
            'link' => env('branch').'/enquiry_leads/'
        ];

        return View('branch.enquiry_leads.index',$data);
    }
    ...
    */
    public function not_replied(){
        ...
        return View('branch.enquiry_leads.not_replied',$data);
    }
    ...
    */
    public function assign(Request $request,$id){
        ...
        return Redirect(env('branch').'/enquiry_leads')->with('message','Enquiry Lead Assigned Successfully.');
    }
    ...
    */
    public function edit($id){
        ...
        if($enquiry_leads->status > 1){
            return Redirect(env('branch').'/enquiry_leads')->with('error','Sorry! Already Called Number cannot be Edited');
        }
        ...
        return View('branch.enquiry_leads.edit',$data);
    }
    ...
    */
    public function update(Request $request,$id){
        ...
        if($data->validate($request->all(),"edit")){
            return Redirect(env('branch').'/enquiry_leads/'.$id.'/edit')->withErrors($data->validate($request->all(),"edit"))->withInput();
        }elseif($data->duplicateChk("edit",$request,$id)){
            return Redirect(env('branch').'/enquiry_leads/'.$id.'/edit')->with('error','Sorry! '.$data->duplicateChk("edit",$request,$id).' Already Exists')->withInput();
        }
        ...
        return Redirect(env('branch').'/enquiry_leads')->with('message','Enquiry Lead Updated Successfully.');
    }
    ...
    */
    public function destroy($id){
        ...
        if($enquiry_leads->status > 1){
            return Redirect(env('branch').'/enquiry_leads')->with('error','Sorry! Already Called Number cannot be Deleted');
        }

        EnquiryLeads::where('id', $id)->delete();

        return Redirect(env('branch').'/enquiry_leads')->with('message','Enquiry Lead Deleted Successfully.');
    }
}
