...
class Branch extends Authenticatable
{
	...
	*/
	public function matchPassword($password){
	  	if(Auth::guard('branch')->attempt(['email' => Auth::guard('branch')->user()->email, 'password' => $password])){
			return false;
	  	}else{
		  	return true;
	  	}
	...
	public function duplicateChk($type,$request,$id=0){
		...
		if($type == "add"){
			if(Branch::where('email',$email)->exists()){
				return "Email";
			}elseif(($phone && Branch::where('phone',$phone)->exists())){
				return "Phone";
			}else{
				return false;
			}
		}else{
			if(Branch::where('id','!=',$id)->where('email',$email)->exists()){
				return "Email";
			}elseif(($phone && Branch::where('id','!=',$id)->where('phone',$phone)->exists())){
				return "Phone";
			}else{
				return false;
			}
