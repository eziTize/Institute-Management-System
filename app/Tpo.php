...
class Tpo extends Authenticatable
{
    protected $table = "tpo";
	...
	*/
	public function matchPassword($password){
	  	if(Auth::guard('tpo')->attempt(['email' => Auth::guard('tpo')->user()->email, 'password' => $password])){
			return false;
	  	}else{
		  	return true;
	  	}
	...
	*/
	public function duplicateChk($type,$request,$id=0){
		...
		if($type == "add"){
			if(Tpo::where('email',$email)->exists()){
				return "Email";
			}elseif(($phone && Tpo::where('phone',$phone)->exists())){
				return "Phone";
			}else{
				return false;
			}
		}else{
			if(Tpo::where('id','!=',$id)->where('email',$email)->exists()){
				return "Email";
			}elseif(($phone && Tpo::where('id','!=',$id)->where('phone',$phone)->exists())){
				return "Phone";
			}else{
				return false;
			}
